...
#### Semantic network analysis {.tabset}
...
To assess the efficiency of the different models the documents from the main data set were:

* Split into two subset, a train set and test set, following a 80%-20% ratio
* The train set contained 3'415'739 documents and the test set 853'939 documents
...
### 2.3.3 Processing time vs accuracy
A balance needed to be reached between processing time versus accuracy. In ngrams models, the amount of ngram2 and ngram3 quickly become huge, increasing drastically the processing time needed for the algorithms to produce a result. As an example, for the whole train set, after considering only 95% of the vocabulary, the amount of ngram1 amounts to 19'937, to more than 7 millions for ngram2 and to more than 28 millions for ngram3. Therefore, despite the reduction in vocabulary size and the optimization done on both algorithms, it still required more than 127 seconds for the SBO function or 180 second to the KBO function to return a result.

The methodology adopted to solve this problem was to reduce the total amount of n-grams to be processed and to find a threshold at which the processing time would be smaller than 1 second, threshold deemed acceptable for the user of a mobile device. This was done by:

* Covering only 95% of the vocabulary, which means moving in the train set from 683'158 unique words to 19'937 (ngram1)
* Further reducing the amount of ngram2 and ngram3 by finding the best minimal frequency threshold in regard to processing time (< 1 sec.)

The following table highlights that the optimal minimal frequency for both ngram2 and ngram3 is 21. It appears to be a good balance between accuracy and processing time, as it allows good accuracy and results to be returned in less than 1 second (0.586). This table was made on a sample from 10 test sentences. Therefore, the percentage of success from both algorithms should not be considered as significant. 

Nevertheless, the accuracy trend highlights a very interesting point: despite the reduction in the number of ngram2 and ngram3, the accuracy paradoxically increased from 20% to 30%. The explanation is that given the fact that a sequence analyzed is at best limited to three words (ngram3), the probability of having a third word followed by two other words was higher for a certain ngram3. When the reduction operated on ngram3 removed this sequence, it activated the back off model which finally produced the correct result on a shorter sequence. 

This point emphasizes the weakness of an approach based on n-grams model, limited to a sequence of maximum three words due to processing time constraints and further not taking into consideration the context of whole sentences or longer sequences. A deep learning model approach will later be developed to address those issues.
