...
* The training / testing procedure is the following:
    + A first samples of 10'000 documents is randomly selected from the train set
    + Both models are trained on this first sample
    + Both models are then tested on the whole test set (853'939 documents)
    + Speed and accuracy are registered
    + A new train sample is randomly selected by adding the step of 10'000 to the size of the preceding sample, in this case that means having the new and second sample containing 20'000 documents
    + Both models are trained on this new sample and tested on the whole test set.
    + This sequential procedure of increasing training sample size with steps of 10'000 is repeated until reaching the size of the whole train set (3'415'739 documents)

It should be noted that to increase the computing speed of both model procedures, the ngram3 and ngram2 with a frequency of 1 were removed. This means that much memory could be saved and back-off models were activated more quickly when needed. This in the end drastically increased the computing speed of both models. 

* For a test conducted on random sample of 10000 documents, it increased the computing speed by:
    + 51 times for the SBO model
    + 26 times for the KBO model
