...
# Data
        # Clean the processed data dir
                # Check if data file present in procesed dir
                        # If present is deleted

        # processedPath <- ("./data/processedData/")
        # if (file.exists(paste0(processedPath,"en_US.blogs.txt" ))){unlink(paste0(processedPath,"en_US.blogs.txt"))}
        # if (file.exists(paste0(processedPath,"en_US.news.txt" ))){unlink(paste0(processedPath,"en_US.news.txt"))}
        # if (file.exists(paste0(processedPath,"en_US.twitter.txt" ))){unlink(paste0(processedPath,"en_US.twitter.txt"))}
        
        # # Copy the raw data files into the cleaned processed dir
        # file.copy("./data/rawData/en_US/en_US.blogs.txt", "./data/processedData")
        # file.copy("./data/rawData/en_US/en_US.news.txt", "./data/processedData")
        # file.copy("./data/rawData/en_US/en_US.twitter.txt", "./data/processedData")

        # # Get and check that can be loaded without errors
        # blogsDf <- read.table(paste0(processedPath, "en_US.blogs.txt"), sep = "\n", quote="", fill=F, header = F, colClasses = "character", encoding="UTF-8")
        # newsDf <- read.table(paste0(processedPath, "en_US.news.txt"), sep = "\n", quote="", fill=F, header = F, colClasses = "character", encoding="UTF-8")
        # twitterDf <- read.table(paste0(processedPath, "en_US.twitter.txt"), sep = "\n", quote="", fill=F, header = F, colClasses = "character", skipNul = TRUE, encoding="UTF-8")
        # 
               # Preprocess txt data to remove quotes and avoid importation error into R (quote transformed into â)
                        # gsub('”', '', blogsDf)
                        # noquote(paste0(processedPath,"en_US.blogs.txt"))
...
# DataFrame creation
#=======================
        # This ease the cleaning process and subsequent transfo into a corpus format
...
cat(green("Individual corpuses merged into one large corpus\n"))
